...
import { trailblazerHarmony, trailblazerHarmonyHandlerFactory } from '../../data/characters';
...
describe('Trailblazer Harmony Scenario Test', () => {
    let initialState: GameState;
    const tbId = 'trailblazer-harmony-1';
    const allyId = 'ally-1';
    ...
    beforeEach(() => {
        ...
        const { handlerMetadata, handlerLogic } = trailblazerHarmonyHandlerFactory(tbId, 80, 0);
        initialState = {
            ...initialState,
    ...
    describe('Skill - Super Break Buff', () => {
        it('should apply super break buff to team', () => {
            let state = initialState;

            state = { ...state, currentTurnOwnerId: createUnitId(tbId) };
            state = dispatch(state, { type: 'SKILL', sourceId: tbId, targetId: tbId });
    ...
    describe('Talent - Break Efficiency', () => {
        it('should have break efficiency passive', () => {
            let state = initialState;
    ...
    describe('Ultimate - Super Break Enhancement', () => {
        it('should enhance super break damage', () => {
            let state = initialState;
            const getTB = (s: GameState) => getUnit(s, tbId);
            ...
            state = {
                ...state,
                registry: state.registry.update(createUnitId(tbId), u => ({ ...u, ep: 140 }))
            };

            state = { ...state, currentTurnOwnerId: createUnitId(tbId) };
            state = dispatch(state, { type: 'ULTIMATE', sourceId: tbId, targetId: tbId });
