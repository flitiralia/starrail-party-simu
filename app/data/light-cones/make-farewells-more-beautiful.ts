...
export const MAKE_FAREWELLS_MORE_BEAUTIFUL: ILightConeData = {
    ...
    eventHandlers: [
        ...
        },
        {
            ...
            handler: (event, state, unit, superimposition) => {
                let lcState = unit.lightConeState?.['make-farewells-more-beautiful'] || { cooldown: 0, activations: 0 };

                if (event.type === 'ON_ULTIMATE_USED' && event.sourceId === unit.id) {
                    lcState = { ...lcState, activations: 0 };
                    const newUnit = { ...unit, lightConeState: { ...unit.lightConeState, 'make-farewells-more-beautiful': lcState } };
                    return {
                        ...state,
                ...
                if (event.type === 'ON_UNIT_DEATH') {
                    ...
                    if (updatedUnit) {
                        const finalUnit = { ...updatedUnit, lightConeState: { ...updatedUnit.lightConeState, 'make-farewells-more-beautiful': lcState } };
                        newState = {
                            ...newState,
