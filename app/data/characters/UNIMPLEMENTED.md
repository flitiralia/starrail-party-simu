...
### longevous-disciple.ts（宝命長存の蒔者）
**仕様**: 「装備キャラがHPを消費した時、または味方によってHPを消費させられた時、会心率+8%。」

**現状**: 敵からの攻撃時のみトリガー。HP消費（刃の自傷など）が検知できない。

**必要な変更**:
1. `ON_HP_CONSUMED` イベントの追加
2. 刃などのHP消費処理でこのイベントを発火
3. 遺物ハンドラでこのイベントを購読

### ninja-record-sound-hunt.ts（忍事録・音律狩猟）
**仕様**: HP変化時に会心ダメージバフを付与。

**現状**: `ON_UNIT_HEALED` と `ON_DAMAGE_DEALT` のみ使用。自傷やHP消費を検知できない可能性あり。

**必要な変更**:
1. `ON_HP_CHANGED` 汎用イベントの検討
2. または `ON_HP_CONSUMED` イベントの追加
...
| ファイル名 | 名称 |
|------------|------|
| `belobog_of_the_architects.ts` | 建築家のベロブルグ |
| `talia_kingdom_of_banditry.ts` | 盗賊のタリア王国 |
| `pan_cosmic_commercial_enterprise.ts` | 汎銀河商事会社 |
| `celestial_differentiator.ts` | 天体階差機 |
...
### passerby-of-wandering-cloud.ts（流雲無痕の過客）
**現状**: 重複不可で実装。原作仕様（重複可）と異なる可能性あり。
