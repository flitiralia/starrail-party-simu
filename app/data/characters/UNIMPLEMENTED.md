...
### 独自削靭ロジックの共通化
**現状**: `dahlia.ts` 内にローカルの `reduceToughness` 関数を定義して使用している。
**必要な変更**: `simulator/engine/toughness.ts` 等に削靭処理を共通化し、撃破効率や耐性を一括管理できるようにする。

### A6: EP回復制限の厳密な管理
**仕様**: 「1ターン内に3回まで回復可能」。
**現状**: 自身のターン開始時にリセットするロジックで実装。追加行動やターンの定義が複雑になった場合の挙動確認が必要。
...
### 超撃破計算式のシステム統合
**現状**: `LEVEL_CONSTANT_80` を用いた固定的な計算式を使用。
**必要な変更**: `calculatorSuperBreakDamage` を使用し、レベル係数テーブルを参照する形式に統一する。

---

## 帰忘の流離人 (Fugue)

### 狐の祈り: 弱点無視削靭と超撃破の相互作用
**仕様**: 弱点無視時は削靭値50%として扱う。
**現状**: 基本ロジックは実装済みだが、多段ヒット攻撃時や複数のバフが重なった際の削靭値計算の正確性を検証する必要がある。
